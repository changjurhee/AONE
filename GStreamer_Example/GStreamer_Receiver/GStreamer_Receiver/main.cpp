...
int main(int argc, char* argv[]) {
    ...
    // GStreamer 파이프라인 생성
    GstElement* pipeline = gst_pipeline_new("receiver_pipeline");
    GstElement* src = gst_element_factory_make("udpsrc", "src");
    GstElement* capsfilter = gst_element_factory_make("capsfilter", "filter");
    GstElement* pay = gst_element_factory_make("rtph264depay", "pay");
    GstElement* dec = gst_element_factory_make("avdec_h264", "dec");
    GstElement* sink = gst_element_factory_make("autovideosink", "sink");

    // 파이프라인에 요소 추가
    gst_bin_add_many(GST_BIN(pipeline), src, capsfilter, pay, dec, sink, NULL);

    // 소스 요소에 송신 주소 설정
    g_object_set(src, "port", 5001, NULL);

    // 요소 간 링크 설정
    gst_element_link_many(src, capsfilter, pay, dec, sink, NULL);
